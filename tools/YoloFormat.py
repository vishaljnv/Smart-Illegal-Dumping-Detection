...
class VOC:
    ...
    """
    def __init__(self, cls_list_path, cls_list_arr):
        with open(cls_list_path, 'r') as file:
            l = file.read().splitlines()

        self.cls_list = l
        self.cls_list_arr = cls_list_arr
        ...
        try:
            ...
            for filename in filenames:
                ...
                for _object in objects:
                    #only add objects in the classes we care about
                    cont_flag = 0
                    tmp = {
                    ...
                    #print(tmp["name"])
                    for k in range(0, len(self.cls_list)):
                        #print(tmp["name"], self.cls_list[k])
                        if tmp["name"] == self.cls_list[k]:
                              #print(k)
                              tmp["name"] = self.cls_list_arr[k]
                              cont_flag = 1
                              #print(tmp["name"])
                              break

                    if cont_flag == 0:
                        #if we don't find a match, subtract 1 from num obj
                        obj["num_obj"] = obj["num_obj"] - 1
                        continue
...
class YOLO:
    ...
    def generate(self, data):
        ...
        try:
            ...
            result = {}
            #use a continue flag to determine whether to skip an object or not
            cont_flag = 0
            #i = 0
		...
		#jskow -> check if data exists at all????
                for idx in range(0, int(data[key]["objects"]["num_obj"])):
                    ...
                    #if the class id of the object is not in our list, then continue
                    obj_name = data[key]["objects"][str(idx)]["name"]
                    print("name: ", obj_name) #self.cls_list)
                    for k in range(0,len(self.cls_list)):
                        print("class name: ", self.cls_list[k], "obj name: ", obj_name)
                        if self.cls_list[k] == obj_name:
                             cont_flag = 1
                             break
